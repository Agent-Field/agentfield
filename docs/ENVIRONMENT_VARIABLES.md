...
- `AGENTFIELD_LOG_STDOUT` (optional, default off): Opts the Python SDK into mirroring structured execution records to stdout as JSON. Set to `1`, `true`, `yes` or `on` (case-insensitive, surrounding whitespace ignored) to enable it; forwarding to the control plane continues unchanged regardless.
