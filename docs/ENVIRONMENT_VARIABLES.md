...
- `AGENTFIELD_LOG_STDOUT` (default: `true`): Controls whether structured SDK execution records are mirrored to stdout. Set to `false` when records are forwarded to the control plane but stdout must remain free of JSON payloads.
