...
- `AGENTFIELD_RESUME_INTERRUPTED_RUNS` (default: `false`): Automatically hand interrupted run roots to new workflow-scope restarts. Equivalent YAML: `agentfield.node_health.resume_interrupted_runs`.
- `AGENTFIELD_RESUME_INTERRUPTED_MAX_ATTEMPTS` (default: `1`): Maximum automatic handoffs in one restart lineage.
- `AGENTFIELD_RESUME_INTERRUPTED_WINDOW` (default: `1h`): Startup-sweep recency window.
- `AGENTFIELD_RESUME_INTERRUPTED_LIMIT` (default: `25`): Maximum startup-sweep candidates.
- `AGENTFIELD_RESUME_INTERRUPTED_DELAY` (default: `15s`): How long the control plane waits for a replacement agent instance to register before dispatching each successor. The delay applies both to the startup sweep and to every inline handoff; set it to at least the time an agent pod takes to become ready.
