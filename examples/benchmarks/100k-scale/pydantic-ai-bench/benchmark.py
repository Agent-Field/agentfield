#!/usr/bin/env python3
"""
Pydantic AI Baseline Benchmark

Measures equivalent operations for fair comparison.
Uses Pydantic AI's tool registration as the comparable operation.
"""
...
try:
    ...
    print("Warning: pydantic-ai not installed. Install with: pip install pydantic-ai")
...
def benchmark_tool_registration(num_tools: int, iterations: int, warmup: int, verbose: bool) -> list[float]:
    """Measure time to create Pydantic AI tools."""
    ...
    for i in range(iterations + warmup):
        ...
        agent = Agent('test-model', defer_model_check=True)
        ...
        for j in range(num_tools):
            ...
            def make_tool(tool_idx):
                ...
                tool_func.__name__ = f"tool_{tool_idx}"
                return tool_func

            agent.tool_plain(make_tool(idx))
...
def benchmark_memory(num_tools: int, iterations: int, warmup: int, verbose: bool) -> list[float]:
    """Measure memory footprint of Pydantic AI tools."""
    if not PYDANTIC_AI_AVAILABLE:
        return []
...
def benchmark_cold_start(iterations: int, warmup: int, verbose: bool) -> list[float]:
    """Measure time to create a minimal Pydantic AI setup."""
    if not PYDANTIC_AI_AVAILABLE:
        return []
...
def benchmark_tool_invocation(num_tools: int, num_invocations: int, verbose: bool) -> list[float]:
    """Measure tool invocation latency (direct function call, not via LLM)."""
    if not PYDANTIC_AI_AVAILABLE:
        return []
    ...
    # Create simple functions to benchmark raw invocation overhead
    funcs = []
    for i in range(num_tools):
        ...
        def make_func(tool_idx):
            def tool_func(query: str) -> dict:
                return {"tool_id": tool_idx, "processed": True, "timestamp": time.time_ns()}
            return tool_func

        funcs.append(make_func(idx))

    # Warm up
    for i in range(min(1000, num_invocations // 10)):
        funcs[i % num_tools]("test")

    # Measure
    results = []
    for i in range(num_invocations):
        func_idx = i % num_tools
        start = time.perf_counter()
        funcs[func_idx]("test")
...
def main():
    parser = argparse.ArgumentParser(description="Pydantic AI Baseline Benchmark")
    ...
    if not PYDANTIC_AI_AVAILABLE:
        print("Error: pydantic-ai not available")
        return
    ...
    if verbose:
        print("Pydantic AI Baseline Benchmark")
        print("==============================")
