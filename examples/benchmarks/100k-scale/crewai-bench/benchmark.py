#!/usr/bin/env python3
"""
CrewAI Baseline Benchmark

Measures equivalent operations for fair comparison.
Uses CrewAI's tool/agent registration as the comparable operation.
"""
...
def benchmark_tool_registration(num_tools: int, iterations: int, warmup: int, verbose: bool) -> list[float]:
    """Measure time to create CrewAI tools."""
...
def benchmark_cold_start(iterations: int, warmup: int, verbose: bool) -> list[float]:
    """Measure time to create a minimal CrewAI setup."""
    if not CREWAI_AVAILABLE:
        return []
...
def benchmark_tool_invocation(num_tools: int, num_invocations: int, verbose: bool) -> list[float]:
    ...
    for i in range(num_tools):
        ...
        def make_tool(query: str) -> dict:
            """Process query."""
            return {"tool_id": idx, "processed": True, "timestamp": time.time_ns()}

        tools.append(make_tool)

    # Warm up
    for i in range(min(1000, num_invocations // 10)):
        tools[i % num_tools].run("test")
...
def main():
    parser = argparse.ArgumentParser(description="CrewAI Baseline Benchmark")
    ...
    if verbose:
        print("CrewAI Baseline Benchmark")
        print("=========================")
