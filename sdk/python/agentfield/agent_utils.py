    ...
    def is_port_available(port: int) -> bool:
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                # SO_REUSEADDR, so a port whose previous connections are still
                # in TIME_WAIT (every restart within ~60 s of a graceful stop)
                # is available to it. Without this option the check rejects
                # exactly the port the control plane assigned and polls.
