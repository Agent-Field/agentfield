...
func (h *ConfigHandler) GetConfigSchemaHandler(c *gin.Context) {
	...
	if agentID == "" {
		handlers.RespondBadRequest(c, "agentId is required")
		return
	}
	...
	if packageID == "" {
		handlers.RespondBadRequest(c, "packageId query parameter is required")
		return
	}
	...
	if err != nil {
		handlers.RespondNotFound(c, "package not found")
		return
	}
	...
	if err := json.Unmarshal(agentPackage.ConfigurationSchema, &schema); err != nil {
		handlers.RespondInternalError(c, "failed to parse configuration schema")
		return
	}
...
func (h *ConfigHandler) SetConfigHandler(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		handlers.RespondBadRequest(c, "invalid request body: "+err.Error())
		return
	}
	...
	if err != nil {
		handlers.RespondInternalError(c, "failed to validate configuration")
		return
	}
	...
	if err != nil {
		...
		if err := h.storage.StoreAgentConfiguration(ctx, newConfig); err != nil {
			handlers.RespondInternalError(c, "failed to store configuration")
			return
		}
		...
		if err := h.storage.UpdateAgentConfiguration(ctx, existingConfig); err != nil {
			handlers.RespondInternalError(c, "failed to update configuration")
			return
		}
