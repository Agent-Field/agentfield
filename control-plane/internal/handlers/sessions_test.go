...
func TestStartSessionHandlerCreatesSessionForRegisteredDefinition(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/sessions/support.voice/start", bytes.NewBufferString(`{"provider":"openai","transport":"webrtc"}`))
...
func TestStartSessionHandlerRejectsUnsupportedTransport(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/sessions/support.voice/start", bytes.NewBufferString(`{"provider":"openrouter","transport":"webrtc"}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
